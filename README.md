...
* **FontCreator Data Files and Registry Keys.md**: Includes data files and registry keys for FontCreator.
>>>>>>> 7ea6a30 (Translated Sobreclick to German)
...
## Fonts
Created with CrystalTile2 and edited with FontCreator. All except LTROL Font and ZOE TFOM are released under the terms of the WTFPL license (see the license agreement in the font files). All fonts have only a regular type style, so don't apply bold or italic attributes, otherwise the OS will synthesise styles, which will bloat PDF files.
...
Adding these user agents in the files below will show them in Otter Browser's user agent menu. The browser itself is mentioned in [my document on changing the user agent in web browsers](https://docs.google.com/document/d/1hzVWH-akdXdz8cP12RGRekNvbcgmBWZHKtbWec8MTzE/edit).
...
* **Source Dir:** GitHub Repository `mozilla/community-data/credits` (`names.csv` file. No CVS commits from August 1999 until June 2009)
