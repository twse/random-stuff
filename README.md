...
1. Clone the `mozilla/community-data` repository.
2. Rebase the repository with a higher number to edit all revisions to display the entire log.
...
* **This page can't be displayed** (Internet Explorer 11) – In this page is a button labeled **Fix connection problems** (pressing in the browser itself when the original page is accessed as `res://ieframe.dll/dnserror.htm` opens the network diagnostic tool and makes the option from the **Tools** menu not dimmed). The page title changes to **You're not connected to the internet** if a computer is not connected to the internet.
