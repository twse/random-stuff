...
* Version 15.0.0.3048 (2026-02-06) is the last version with Windows 32-bit and 64-bit builds.
* Version 16.0.0.3060 (2026-04-29) disunified the Home, Standard and Professional editions to a single edition.
...
|---------------------------------------------------|----------------|
| Windows 7 and later, macOS 11 "Big Sur" and later | 16.0.0.3077    |
...
* **FontCreatorSetup.exe** – Installs the Win32 version of the program (until version 15.0.0.3048). Use the `/DIR` switch to install the program to another directory.
...
* **FontCreator YYYYMMDD HHMMSS ###.dat/prd** (version 12.0) – Contains crash info (located in `ProgramData\High-Logic\Errors` – **YYYYMMDD HHMMSS** is the computer's date and time and **###** is a number). The Call Stack Information was removed in version 12.0.0.2563 (2020-02-21).
...
### Edit (version 8.0 and later to selected glyphs; used in Paste Special)
* **PasteSpecialAnchors** – This will paste anchors. Default is on (old versions) or off (new versions).
* **PasteSpecialCodepoints** – This will paste codepoints. Default is on (old versions) or off (new versions).
...
* **PasteSpecialMetrics** – This will paste metrics. Default is on (old versions) or off (new versions).
* **PasteSpecialMetricsOption** – Choose whether to paste metrics. Default is 0 (Side bearings).
* **PasteSpecialOutlineData** – This will paste outline data. Default is on (old versions) or off (new versions).
...
* **OpenFriendlyGlyphNames** (version 11 and later) – Used with **Open Font: Generate friendly glyph names**. Default is on.
...
* **NewFontIncludeOutlines** (version 5.6 and later) – Used with **Predefined outlines: Include outlines** in the New Font window (the outlines are from Garava). Default is on (version 5.6–8.0) and off (version 9.0 and later).
...
* **OTLFDesignerPreviewSampleText** – Contains sample text used in the preview text. Default is **The five boxing wizards jump quickly** (old versions) or empty (new versions).
...
* **OverviewFontZoom** – Zoom glyphs in the font overview. Default is 35 (old versions) or 48 (new versions).
...
* **FeatureTags** (version 7.0 and later) – Select OpenType features to use. Default is empty (old versions) or _shaper (new versions).
...
* **PreviewSampleFontSize** – Scale the font in the Preview window. Default is 24 (old versions) or 32 (old versions).
* **PreviewSampleText** – Contains sample text in the Preview window. Default is **High-Logic ©** or **&lt;enter preview text here&gt;** (old versions) or **The quick brown fox.** (new versions)
...
> [!NOTE]
> All resources start with HL (the vendor ID is in the vendor list and is the default vendor ID used for new fonts).
...
### About Dialog
These are located in the corners of the program name, version number, copyright notice and link to the company's website.
